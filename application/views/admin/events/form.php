...
<form method="post" action="<?= isset($row)? site_url('admin/events/'.$row->id.'/update') : site_url('admin/events/store'); ?>">
  <div class="mb-3">
  ...
  <div class="row mb-3">
    <div class="col">
      ...
      <input type="datetime-local" name="start_at" class="form-control" required value="<?= isset($row->start_at)? date('Y-m-d\TH:i',strtotime($row->start_at)) : set_value('start_at'); ?>">
    </div>
    <div class="col">
      <label class="form-label">Selesai</label>
      <input type="datetime-local" name="end_at" class="form-control" required value="<?= isset($row->end_at)? date('Y-m-d\TH:i',strtotime($row->end_at)) : set_value('end_at'); ?>">
    </div>
  </div>
  <div class="mb-3">
    <label class="form-label">Banner URL</label>
    <input type="url" name="banner_url" class="form-control" value="<?= html_escape($row->banner_url ?? set_value('banner_url')); ?>">
  </div>
  <div class="mb-3">
